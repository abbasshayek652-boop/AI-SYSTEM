...
async def start_agent(cmd: Command, request: Request, ctx: object) -> dict[str, object]:
    return await _start_agent(cmd, _ensure_request_app(request), ctx)  # type: ignore[arg-type]


async def stop_agent(cmd: Command, request: Request, ctx: object) -> dict[str, object]:
    return await _stop_agent(cmd, _ensure_request_app(request), ctx)  # type: ignore[arg-type]


async def registry_validate(request: Request, ctx: object) -> dict[str, object]:
    return await _registry_validate(_ensure_request_app(request), ctx)  # type: ignore[arg-type]


async def registry_dryrun(request: Request, ctx: object) -> dict[str, object]:
    return await _registry_dryrun(_ensure_request_app(request), ctx)  # type: ignore[arg-type]
