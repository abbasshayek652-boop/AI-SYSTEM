...
def ensure_backend() -> None:
    ...
    if not owner:
        ...
        while time.monotonic() < deadline:
            ...
            time.sleep(0.25)
        raise RuntimeError(
            f"Gateway did not become healthy on {API_BASE} "
            f"within {STARTUP_TIMEOUT_SECONDS:g}s"
        )
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if process.poll() is not None:
                raise RuntimeError(
                    f"Mother AI gateway exited during startup. See {LOG_FILE}."
                )
            time.sleep(0.25)

        raise RuntimeError(
            f"Mother AI gateway did not become healthy on {API_BASE} "
            f"within {STARTUP_TIMEOUT_SECONDS:g}s"
        )
    except Exception:
        if _BACKEND_PROCESS is not None:
...
def home_page() -> None:
    """Main operator dashboard used as the explicit navigation home page."""
    ...
    st.caption(
        f"{st.session_state.email} · {st.session_state.role} · "
    ...
    runtime_agents = (
        ...
        if isinstance(agent_payload.get("agents"), list)
        else []
    )
    catalog_agents = (
        catalog_payload.get("agents", [])
        if isinstance(catalog_payload.get("agents"), list)
        else []
    )
    ...
    m4.metric(
        "Runtime",
        int(catalog_payload.get("loaded_count", len(runtime_agents))),
    )
    ...
    st.divider()
    tab_runtime, tab_catalog, tab_executive, tab_health = st.tabs(
        ["Runtime Agents", "Complete Catalog", "Mother Executive", "System Health"]
    )

    with tab_runtime:
        st.subheader("Runtime Agents")
        st.caption(
            "Only agents loaded by registry.json appear here. "
            "Catalog-only capabilities are shown separately."
        )
        ...
        for row in range(0, len(runtime_agents), 2):
            ...
            for column, agent in zip(cols, runtime_agents[row : row + 2]):
                with column:
                    ...
                    mode = (
                        agent.get("mode")
                        or (agent.get("registry") or {}).get("config", {}).get("mode")
                        or "standard"
                    )
                    ...
                    with st.container(border=True):
                        ...
                        st.caption(
                            f"{agent.get('class_name', 'Agent')} · "
                            f"{agent.get('layer', 'Runtime')} · mode: {mode}"
                        )
                        ...
                        with b1:
                            if st.button(
                                "▶ Start",
                                key=f"start_{key}",
                                disabled=running or not can_control,
                        ...
                        with b2:
                            if st.button(
                                "■ Stop",
                                key=f"stop_{key}",
                                disabled=not running or not can_control,
                                use_container_width=True,
                            ):
    ...
    with tab_catalog:
        ...
        st.caption(
            "50 planned capabilities. 'Scaffold' means the interface exists "
            "conceptually but has no business side effects yet."
        )
        layers = list(dict.fromkeys(str(a.get("layer")) for a in catalog_agents))
        layer_filter = st.selectbox("Layer", ["All"] + layers)
        filtered = [
            a for a in catalog_agents
            if layer_filter == "All" or a.get("layer") == layer_filter
        ]
        for agent in filtered:
            ...
            status = (
                ...
                else ("LOADED" if runtime.get("loaded") else implementation.upper())
            )
            with st.container(border=True):
    ...
    with tab_executive:
        ...
        if st.session_state.role in {"operator", "admin"}:
            ...
            targets = st.multiselect(
                ...
                [a.get("key") for a in runtime_agents],
            )
            ...
            if st.button("Create decision", type="primary"):
                ...
                if "error" in result:
                    ...
                    st.success(f"Decision created: {result.get('id')}")
                    st.json(result)
    ...
    with tab_health:
        ...
        st.json(
            {
                ...
                "catalog": {
                    ...
                    "loaded": catalog_payload.get("loaded_count"),
                },
                "executive": executive,
            }
        )
        ...
        with st.expander("Raw catalog JSON"):
            st.code(
                json.dumps(catalog_payload, indent=2, default=str),
                language="json",
            )
